...
define( 'CRESTA_WHATSAPP_CHAT_PLUGIN_VERSION', '1.1.8' );
...
function cresta_whatsapp_chat_menu() {
	...
	$cresta_whatsapp_options_page = add_options_page( esc_html__( 'Cresta Help Chat Settings'), 'Cresta Help Chat', 'manage_options', 'cresta-whatsapp-chat.php', 'cresta_whatsapp_chat_option', 81 );
}
...
function cresta_whatsapp_chat_css_top() {
	...
	echo ".cresta-whatsapp-chat-container, .cresta-whatsapp-chat-container-button {z-index:". intval($whatsapp_zindex) ."}";
			...
			(function($) {
				...
				$(document).ready(function() {
					...
					if (mobileDetect) {
						...
						$('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)."').on('click', function(){
							window.location = 'whatsapp://send?text=&phone=".esc_attr($theNumber)."&abid=".esc_attr($theNumber)."';
						});
					} else {
						$('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)."').on('click', function(){
							if ($('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)." .cresta-whatsapp-chat-container-button').hasClass('open')) {
								$('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)." .cresta-whatsapp-chat-container-button').removeClass('open');
							} else {
								$('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)." .cresta-whatsapp-chat-container-button').addClass('open');
								$('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)." textarea.cresta-whatsapp-textarea').focus();
							}
						});
						$('.cresta-whatsapp-chat-container-button').on('click', function(event){
							event.stopPropagation();
						});
						$('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)." .cresta-whatsapp-chat-container-button .cresta-whatsapp-send').click(function() {
							var baseUrl = 'https://web.whatsapp.com/send?phone=".esc_attr($theNumber)."&text=';
							var textEncode = encodeURIComponent($('.cresta-whatsapp-chat-button.click-".esc_attr($random_number)." .cresta-whatsapp-textarea').val());
	...
	<div class="wrap">
		...
		<div id="poststuff">
			...
			<form method="post" action="options.php">
				...
				$cwc_options = get_option( 'crestawhatsappchat_settings' );
				?>
				<span class="description attributes notice"><?php echo wp_kses_post('<b>Please note:</b> remember that the WhatsApp number will be <b>visible and public to all users</b> of your website. Please <b>do not</b> use a number that you want to keep private.', 'cresta-whatsapp-chat'); ?></span>
					...
					<tbody>
						...
						<tr valign="top" class="chkifgroup">
							...
							<td>
								...
								printf( esc_html__( 'Add your phone number with the country code, take a look at %1$s this page %2$s to find the correct country code.', 'cresta-whatsapp-chat' ), '<a href="https://faq.whatsapp.com/en/general/21016748" target="_blank">', '</a>' );
								?>
								</span>
							</td>
						</tr>
				...
				<div id="postbox-container-1" class="postbox-container">
					<div class="meta-box-sortables">
						...
						<div class="postbox" style="border: 4px solid #25d366;">
							...
							<div class="inside">
								<a href="https://crestaproject.com/downloads/cresta-help-chat/?utm_source=plugin_whatsapp&utm_medium=moreinfo_meta" target="_blank" alt="Get Cresta Help Chat PRO"><img src="<?php echo esc_url(plugins_url( '/images/banner-cresta-whatsapp-chat-pro.png' , __FILE__ )); ?>"></a><br/>
								<?php echo wp_kses_post( 'Get <strong>Cresta Help Chat PRO</strong> for only', 'cresta-whatsapp-chat' ); ?> <strong>9,99&euro;</strong><br/>
								<ul>
						...
						<div class="postbox" style="border: 4px solid #0084ff;">
                            ...
                            <h3><span><div class="dashicons dashicons-admin-plugins"></div> Cresta Social Messenger Plugin</span></h3>
                            <div class="inside">
                                <a href="https://crestaproject.com/downloads/cresta-facebook-messenger/" target="_blank" alt="Get Cresta Social Messenger"><img src="<?php echo plugins_url( '/images/banner-cresta-facebook-messenger.png' , __FILE__ ); ?>"></a><br/>
								With Cresta Social Messenger you can allow your users or customers to contact you via <strong>Facebook Messenger</strong> simply by clicking on a button.<br/>
								...
								<a class="crestaButton" href="https://crestaproject.com/downloads/cresta-facebook-messenger/" target="_blank" title="Cresta Social Messenger">Available in FREE and PRO version</a>
                            </div> <!-- .inside -->
                         </div> <!-- .postbox -->
...
function crestawhatsappchat_get_the_current_ID() {
	...
	$theID = apply_filters( 'cresta_hel_chat_post_id', $theID );
	$theID = $theID ? $theID : '';
	return $theID;
}
