...
<!--    <h2 >Exámenes de <span style="font-size:5em; margin-bottom: 1em"><?=$familiar["nombre"]?> </span></h2>-->
		...
		<tr >
			...
			<?php
				...
				foreach($tests as $examen){
					...
					foreach($inter as $it){
						if($it["competencia_id"]==$examen["competencia_id"]&&$it["edad_id"]==$examen["edad_id"]){
							
							if($examen["resultado"]>$it["intermedio"]){
								$icon= $star;
							}elseif($examen["resultado"]>$it["minimo"]){
								$icon= $bicycle;
							}else{
								$icon= "".$bell."";
							}
					...
					// $vect=0;
					// $empt=0;
					// for($i=0;$i<$resultado;$i=$i+10){
						// $vect+=1;
						// $empt++;
					// }
					// $vect='<img src="../web/images/emoji-laughing.svg" style="width:30px">x'.$vect;
					
					
					$vect="";
					$empt=0;
					for($i=0;$i<$resultado;$i=$i+10){
						$vect.='<img src="../web/images/emoji-laughing.svg" style="width:20px">&nbsp;';
						$empt++;
					}
					for($i=$empt;$i<6;$i++){
						$vect.='<img src="../web/images/emoji-laughing-empty.svg" style="width:20px">&nbsp;';
					}					
					
					if($fechaant!=$examen["fecha"]){					
						if($i>0){
							echo "</td><td  style='".$tdstyle."'>".$acciones."</td></tr>";
							$alter="#eee";
						}
						echo "<tr style='background-color:".$alter."'><td style='".$tdstyle."'>Neurobaby para ".$examen["edad"]["edad"]." </td>";
						echo "<td style='".$tdstyle."'>".calcularEdadEpocaTexto($familiar["fechanacimiento"],$examen["fecha"])."<br/><span style='color:grey; font-size: 0.8em'>(".$examen["fecha"].")</span></td>";
//						echo "<td style='".$tdstyle."'>".$examen["fecha"]."</td>";
						echo "<td style='".$tdstylel."'><p style='padding-bottom: 5%; min-width: 200px'>".$icon."&nbsp;&nbsp;".$examen["competencia"]["competencia"].": <strong>".$vect."</strong></p> ";
					}else{
						
						
						$percent=round($examen["resultado"]*100/60,0);						
						echo "<p style='padding-bottom: 2%; min-width: 200px'>".$icon."&nbsp;&nbsp;".$examen["competencia"]["competencia"].": <strong>".$vect."</strong></p> ";
					}
					$i++;
					$fechaant=$examen["fecha"];
					$acciones="<a href='index.php?r=preguntausuariorespuesta%2Fresultadoagehistorico&familiarid=".$examen["familiar_id"]."&edadid=".$examen["edad_id"]."&fechaid=".$examen["fecha"]."'><img src='../web/images/file-earmark-bar-graph-fill.svg' style='width:2em;' class='testhistorypadding' title='Ver detalle'></a><br/>";
					if($examen["revision"]==0){
						$acciones.="<a href='index.php?r=preguntausuariorespuesta%2Fresultadoagehistorico&familiarid=".$examen["familiar_id"]."&edadid=".$examen["edad_id"]."&fechaid=".$examen["fecha"]."&enviar=true'><img src='../web/images/envelope-paper.svg' style='width:2em' title='Enviar para revisión'></a></br>";
					}else{
						$acciones.="<img src='../web/images/envelope-check-fill.svg' style='width:2em' title='Este examen ya ha sido enviado'>";
					}

				}
				echo "</td><td style='".$tdstyle."'>".$acciones."</td>";
				$cambio=true;
			?>
	</table>
